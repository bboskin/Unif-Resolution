...
(defun fo-simplify (f)
  (match f
    ...
    ((list 'implies e1 e2)
     (simplify-or `(,(fo-simplify (negate e1)) ,(fo-simplify e2))))
...
(defun nnf- (f neg?)
  (match f
    ...
    ((list 'forall vars f)
     (if neg? `(exists ,vars ,(nnf- f t))
       `(forall ,vars ,(nnf- f nil))))
    ((list 'exists vars f)
     (if neg? `(forall ,vars ,(nnf- f t))
       `(exists ,vars ,(nnf- f nil))))
    ((list* 'and fs)
     (if neg? `(or . ,(mapcar #'(lambda (e) (nnf- e neg?)) fs))
       `(and . ,(mapcar #'(lambda (e) (nnf- e neg?)) fs))))
    ((list* 'or fs)
     (if neg? `(and . ,(mapcar #'(lambda (e) (nnf- e neg?)) fs))
       `(or . ,(mapcar #'(lambda (e) (nnf- e neg?)) fs))))
    ((list 'implies e1 e2)
     (if neg?
...
(defparameter test8
  '(iff (or (implies (R c3) (forall (x) (P x)))
	    (forall (x) (Q x)))
	(forall (x) (or (implies (R c3) (forall (z) (P z)))
			(Q x)))))
...
(defun fo-no=-val (f)
  (print "Performing simplification, skolemization, prenex, and cnf")
